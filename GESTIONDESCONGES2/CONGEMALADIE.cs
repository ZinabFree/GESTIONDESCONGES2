...
{
    public partial class CONGEMALADIE : Form
    {
        public CONGEMALADIE()
        {
            InitializeComponent();
        ...
        CBASE c = new CBASE();
        private void CONGEMALADIE_Load(object sender, EventArgs e)
        {
            Increament_N();
            ttype.Enabled = false;
            dateD.Enabled = false;
        ...
        private void badd_Click(object sender, EventArgs e)
        {
            try
            {
                if (badd.Text == "Ajouter")
                {
                    ...
                    {
                        verifier_conge();
                        c.chargeGridAgentM(int.Parse(tdot.Text), 3, dataGridView1);
                        badd.Text = "Nouveau";
                    }
                }
                else
        ...
        public void remplire_gride()
        {
            ...
            {
                c.connecter();
                SqlDataAdapter da = new SqlDataAdapter("select N_conge, N_dote, Nom_employe, prenom_employe, date_D, date_F, Nb_jour, Date_saisie, id_type_c, type_de_conge, N_dote_remplacement, Nom_remplacement, Prenom_remplacement from Conge where id_type_c=3 ", c.con);
        ...
        public void verifier_conge()
        {
            ...
            {
                c.connecter();
                SqlDataAdapter da = new SqlDataAdapter("select top 1 * from Conge where id_type_c=3 and N_dote='" + tdot.Text + "' and((GETDATE() between date_D and date_F) or (GETDATE()=date_F)) order by date_F desc ", c.con);
                DataTable dt = new DataTable();
        ...
        public void ajouter_conge()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("insert into conge (N_conge,N_dote, Nom_employe, prenom_employe, date_D, date_F, Nb_jour, Date_saisie, id_type_c,type_de_conge, N_dote_remplacement, Nom_remplacement, Prenom_remplacement) values('" + tnconge.Text + "','" + tdot.Text + "','" + tnomAR.Text + "','" + tprenomAR.Text + "','" + dateD.Text + "','" + dateF.Text + "','" + tnombre_jour.Text + "',getdate(),3,'" + ttype.Text + "','" + tdoteR.Text + "','" + tnomRAR.Text + "','" + tprenomRAR.Text + "')", c.con);
                c.connecter();
                cmd.ExecuteNonQuery();
                c.deconnecter();
                //verifier_conge();
        ...
        private void bdelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("delete conge where N_dote='" + tdot.Text + "' and Nom_employe ='" + tnomAR.Text + "' and date_D='" + dateD.Text + "' and date_F='" + dateF.Text + "' ", c.con);
                    c.connecter();
                    cmd.ExecuteNonQuery();
                    c.deconnecter();
                    clear();
        ...
        private void tnomFR_TextChanged_1(object sender, EventArgs e)
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from employe", c.con);
            DataSet ds = new DataSet();
            da.Fill(ds, "emp");
        ...
        private void tnomR_TextChanged_1(object sender, EventArgs e)
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from employe", c.con);
            DataSet ds = new DataSet();
            da.Fill(ds, "emp");
        ...
        private void tnomFR_TextChanged_2(object sender, EventArgs e)
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from employe", c.con);
            DataSet ds = new DataSet();
            da.Fill(ds, "emp");
        ...
        private void tnomR_TextChanged_2(object sender, EventArgs e)
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from employe", c.con);
            DataSet ds = new DataSet();
            da.Fill(ds, "emp");
